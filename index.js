...
// Owner numbers
const ownerNumber = ["923094300615"];
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes(" 𝗦𝗼𝗻𝘂 𝗣𝗿𝗮𝗰𝗵𝗮-MD~")) {
    console.log("❌ Invalid  𝗦𝗼𝗻𝘂 𝗣𝗿𝗮𝗰𝗵𝗮-MD session format");
    process.exit(1);
  }
  ...
  console.log("♻️  𝗦𝗼𝗻𝘂 𝗣𝗿𝗮𝗰𝗵𝗮-MD session restored successfully");
}
...
app.get('/', (req, res) => {
  ...
  if (fs.existsSync(htmlPath)) {
    ...
    res.send(`
      ...
      <html>
      <head>
        <title> 𝗦𝗼𝗻𝘂 𝗣𝗿𝗮𝗰𝗵𝗮-MD Bot</title>
        <style>
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return https://i.ibb.co/j9bxgf5j/FB-IMG-1785695038321.jpg'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖  𝗦𝗼𝗻𝘂 𝗣𝗿𝗮𝗰𝗵𝗮-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅  𝗦𝗼𝗻𝘂 𝗣𝗿𝗮𝗰𝗵𝗮-MD Bot connected to WhatsApp!");
      ...
      const aliveMsg = `*╭──────────────●●►*\n> * 𝗦𝗼𝗻𝘂 𝗣𝗿𝗮𝗰𝗵𝗮-MD CONNECTED SUCCESSFULLY*\n\n> *Type ${prefix}menu to view commands*  \n\n*╭⊱✫  𝗦𝗼𝗻𝘂 𝗣𝗿𝗮𝗰𝗵𝗮 MD ✫⊱╮*\n*│✫📂 Bot Name: ${botConfig.BOT_NAME}*\n*│✫🛡️ Owner: ${config.OWNER_NAME}*\n*│✫♻️ Prefix: ${prefix}*\n*│✫🌍 Mode: ${config.MODE}*\n*│✫⏰ Uptime: ${runtime(process.uptime())}*\n*╰──────────────●●►*\n\n> Enjoy Using  𝗦𝗼𝗻𝘂 𝗣𝗿𝗮𝗰𝗵𝗮 MD`;
      
      // Image URL for connection message
      const imageUrl = 'https://i.ibb.co/j9bxgf5j/FB-IMG-1785695038321.jpg
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '923094300615@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
